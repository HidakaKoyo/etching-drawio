...
hash handoff と lock が競合を**防止**できるのは、この contract に従う協調 writer (etch CLI、etching skill 経由のエージェント) 同士に限る。draw.io Desktop など lock を守らない writer に対しては、S2 の照合と S6 の照合の間の窓を閉じきれない。
...
- **v1 では旧世代を自動削除しない。**
- 自動回収の対象は **自プロセスが作った `.tmp` 世代のみ**。他プロセスの `.tmp` は経過時間だけを根拠に削除しない。扱うなら、作成者 PID の lock ownership とそのプロセスの消滅確認を必要条件とする
- 完結した旧世代の削除は明示コマンド `etch gc` によるユーザー操作とする
